...
function Send-McpMessage {
    param([Parameter(Mandatory)][hashtable]$Message, [string]$Session = "")

    $headers = @{ Accept = "application/json, text/event-stream" }
    if ($Session) { $headers["Mcp-Session-Id"] = $Session }
    $response = Invoke-WebRequest -Uri $endpoint -Method Post -Headers $headers `
        -ContentType "application/json" -Body ($Message | ConvertTo-Json -Depth 20 -Compress) `
        -TimeoutSec 20 -UseBasicParsing
    $dataLines = @($response.Content -split "`n" | Where-Object { $_ -like "data:*" })
    $json = if ($dataLines.Count) { $dataLines[-1].Substring(5).Trim() | ConvertFrom-Json -Depth 30 }
...
function Invoke-McpTool {
    param([int]$RequestId, [string]$Name, [hashtable]$Arguments, [string]$Session)

    $reply = Send-McpMessage -Session $Session -Message @{
        jsonrpc = "2.0"; id = $RequestId; method = "tools/call"
        params = @{ name = $Name; arguments = $Arguments }
    }
    if ($reply.Json.result.isError) { throw "$Name returned an MCP error." }
...
$null = Send-McpMessage -Session $session -Message @{ jsonrpc = "2.0"; method = "notifications/initialized" }
$sessions = Invoke-McpTool -RequestId 2 -Name "session_manage" -Arguments @{ op = "list"; params = @{} } -Session $session
if ($sessions.count -lt 1) { throw "Godot AI is running, but no Godot editor session is connected." }
