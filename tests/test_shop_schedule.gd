...
func test_three_retail_stores_and_recycling_are_open_each_day() -> void:
	for day in range(1, 22):
		...
		assert_has(open_stores, SlotDemoCatalog.STORE_RECYCLING)
...
func test_first_three_nights_match_slot_card_demo_routes() -> void:
	assert_eq(ShopSchedule.open_store_ids(1), [
		...
		SlotDemoCatalog.STORE_RECYCLING,
	])
	assert_eq(ShopSchedule.open_store_ids(2), [
		SlotDemoCatalog.STORE_TOY,
		SlotDemoCatalog.STORE_RECORD,
		SlotDemoCatalog.STORE_BOOK,
		SlotDemoCatalog.STORE_RECYCLING,
	])
...
func test_no_store_rests_for_two_consecutive_days() -> void:
	for store_id in SlotDemoCatalog.STORE_IDS:
		for day in range(1, 22):
			assert_true(
...
func test_weekday_and_next_open_day_wrap_after_sunday() -> void:
	...
	assert_eq(ShopSchedule.next_open_day(SlotDemoCatalog.STORE_TOY, 2), 3)
	assert_eq(ShopSchedule.next_open_day(SlotDemoCatalog.STORE_TOY, 7), 8)
