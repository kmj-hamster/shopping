...
const EXPECTED_ITEM_IDS: Array[StringName] = [
	...
	&"concrete_city_vol_2", &"nocturne_published", &"shopping_card",
]

const EXPECTED_ILLUSTRATED_ITEM_IDS: Array[StringName] = [
	&"kaleidoscope", &"lotus_candle", &"mung_bean_cake", &"tin_frog", &"ratty_doll",
	&"milkshake", &"fruit_wine", &"fries", &"nuggets",
	&"jasmine", &"camellia", &"cactus", &"foam_fake_flower", &"gardenia",
	&"candy_fake_flower", &"goldberg_variations", &"debussy_clair_de_lune",
...
func test_manifest_is_the_live_content_whitelist_without_test_cards() -> void:
	...
	assert_eq(manifest.expedition_rooms.size(), 9)
	for shape_id in CardPropertySet.SHAPES:
		assert_eq(
			int(manifest.initial_shape_levels.get(shape_id, -1)),
			int(EXPECTED_SHAPE_LEVELS[shape_id]),
		)
	...
	for raw_item in manifest.items:
		...
		if EXPECTED_ILLUSTRATED_ITEM_IDS.has(item.id):
			assert_not_null(item.image, item.id)
			assert_true(
				item.image.resource_path.begins_with("res://resources/items/"),
				item.id,
			)
		else:
			assert_true(EXPECTED_ITEMS_WITHOUT_IMAGES.has(item.id), item.id)
...
func test_retail_cards_match_the_declared_prices_types_and_shapes() -> void:
	_assert_item(&"kaleidoscope", 20, [&"toy"], {&"dream": 2, &"light": 2})
	_assert_item(&"lotus_candle", 40, [&"toy"], {&"tear": 3})
	_assert_item(&"mung_bean_cake", 8, [&"food"], {&"tear": 1})
	_assert_item(&"tin_frog", 8, [&"toy"], {&"light": 1})
	_assert_item(&"ratty_doll", 18, [&"toy"], {&"sleep": 2, &"tear": 1})
	_assert_item(&"milkshake", 16, [&"drink"], {&"sleep": 2})
	_assert_item(&"fruit_wine", 42, [&"drink"], {&"dream": 1, &"sleep": 3})
	_assert_item(&"fries", 8, [&"food"], {&"sleep": 1})
	_assert_item(&"nuggets", 18, [&"food"], {&"dream": 2, &"sleep": 1})
	_assert_item(&"jasmine", 8, [&"flower"], {&"sleep": 1})
	_assert_item(&"camellia", 16, [&"flower"], {&"tear": 2})
	_assert_item(&"cactus", 16, [&"flower"], {&"light": 2})
	_assert_item(&"foam_fake_flower", 10, [&"flower", &"toy"], {&"dream": 1, &"tear": 1})
	_assert_item(&"gardenia", 24, [&"flower"], {&"dream": 2, &"sleep": 2})
	_assert_item(&"candy_fake_flower", 40, [&"flower", &"food"], {&"dream": 3})
	_assert_item(&"goldberg_variations", 20, [&"cassette"], {&"sleep": 2, &"light": 2})
	_assert_item(&"debussy_clair_de_lune", 80, [&"cassette"], {&"dream": 4})
	_assert_item(&"conservatory_story", 16, [&"book"], {&"dream": 2})
	_assert_item(&"nocturne_competition_recording", 12, [&"cassette"], {&"dream": 2, &"tear": 3})
	_assert_item(&"concrete_city_vol_1", 84, [&"book"], {&"dream": 1, &"light": 4})
	_assert_item(&"mirror_and_lamp", 40, [&"book"], {&"light": 3})
	_assert_item(&"ufo_exploration_magazine", 18, [&"book"], {&"dream": 1, &"light": 2})
	_assert_item(&"mania_manga", 10, [&"book"], {&"dream": 1, &"tear": 1})
	_assert_item(&"concrete_city_vol_2", 0, [&"book", &"keepsake"], {&"dream": 4, &"light": 1})
	_assert_item(&"nocturne_published", 0, [&"cassette", &"keepsake"], {&"dream": 1, &"tear": 1})
	_assert_item(&"shopping_card", 0, [], {
		&"dream": 2,
		&"sleep": 2,
		&"tear": 2,
		&"light": 2,
	...
	assert_eq(
		QuestArcCatalog.item_by_id(&"concrete_city_vol_2").image.resource_path,
		"res://resources/items/concrete_city_vol_2.png",
	)
	assert_eq(
		QuestArcCatalog.item_by_id(&"nocturne_published").image.resource_path,
		"res://resources/items/nocturne_premium.png",
	)
	assert_null(QuestArcCatalog.item_by_id(&"shopping_card").image)
	assert_true(QuestArcCatalog.item_by_id(&"shopping_card").allows_all_shapes)
	for event_item_id in [&"concrete_city_vol_2", &"nocturne_published", &"shopping_card"]:
		assert_eq(
			QuestArcCatalog.item_by_id(event_item_id).supply_mode,
...
func test_five_live_recipes_match_requirements_outputs_and_resale_values() -> void:
	var cases := [
		[&"recipe_cold_teddy_bear", &"toy", {&"tear": 5}, &"cold_teddy_bear", [&"toy"], {&"tear": 3}, 20, &"lotus_candle", &"", &"tear"],
		[&"recipe_baby_soothing_bear", &"toy", {&"sleep": 5}, &"baby_soothing_bear", [&"toy"], {&"sleep": 3}, 20, &"ratty_doll", &"", &"sleep"],
		[&"recipe_birthday_cake", &"food", {&"tear": 7}, &"birthday_cake", [&"food"], {&"tear": 4}, 40, &"mung_bean_cake", &"lotus_candle", &"tear"],
		[&"recipe_cola", &"drink", {&"sleep": 7}, &"cola", [&"drink"], {&"sleep": 4}, 40, &"fruit_wine", &"ratty_doll", &"sleep"],
		[&"recipe_rose", &"flower", {&"dream": 3}, &"rose", [&"flower"], {&"dream": 2}, 8, &"jasmine", &"", &"dream"],
	]
	for test_case in cases:
		...
		assert_eq(recipe.required_shapes, test_case[2], recipe.id)
		...
		var totals := SynthesisRules.shape_totals(
			...
			helper,
			test_case[9],
			EXPECTED_SHAPE_LEVELS,
		)
		assert_true(SynthesisRules.evaluate_candidate(recipe, base, totals).is_complete, recipe.id)


func test_disease_recipes_are_outputless_single_shape_paths() -> void:
	var cases := [
		[&"recipe_clear_white_flower_light", &"white_flower", &"light"],
		[&"recipe_clear_white_flower_tear", &"white_flower", &"tear"],
		[&"recipe_clear_wound_sleep", &"expedition_wound", &"sleep"],
		[&"recipe_clear_wound_dream", &"expedition_wound", &"dream"],
	]
	for test_case in cases:
		var recipe := QuestArcCatalog.recipe_by_id(test_case[0])
		assert_not_null(recipe)
		assert_eq(recipe.base_rule.accepted_item_ids, [test_case[1]], recipe.id)
		assert_eq(recipe.required_shapes, {test_case[2]: 3}, recipe.id)
		assert_true(recipe.output_id.is_empty(), recipe.id)
		assert_true(recipe.consumes_without_output, recipe.id)
		assert_true(recipe.escalating_shape_requirement, recipe.id)
...
func test_new_game_starts_with_only_tin_frog_and_keeps_four_shapes() -> void:
	...
	for shape_id in CardPropertySet.SHAPES:
		assert_eq(
			int(state.protagonist_shape_levels.get(shape_id, -1)),
			int(EXPECTED_SHAPE_LEVELS[shape_id]),
		)
