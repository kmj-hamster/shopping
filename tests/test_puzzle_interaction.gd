...
func test_drag_preview_uses_board_scale_and_rotates_around_grabbed_cell() -> void:
	...
	assert_eq(preview.size, Vector2(112, 56))
	assert_eq(preview.position, Vector2(-28, -28))
	...
	assert_eq(preview.size, Vector2(56, 112))
	assert_eq(preview.position, Vector2(-28, -28))
	preview.free()
