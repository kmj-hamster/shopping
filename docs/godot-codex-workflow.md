...
| 输入/交互 | 聚焦 GUT；必要时 MCP 运行场景 + 一次输入模拟 + editor/game 日志 |
| 视觉/UI | CLI 导入 + 相关 GUT + 人工体验；仅在用户明确要求时获取 MCP 游戏截图 |
...
- 升级后依次执行 CLI 检查、MCP 连接、场景写入、运行和日志检查；截图仍需用户明确要求。
- 不在功能开发提交里顺便升级工具。
