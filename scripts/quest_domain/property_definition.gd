...
@export var is_shape_value := false
...
func validation_errors() -> PackedStringArray:
	...
	if is_shape_value and not is_scaled():
		errors.append("Shape %s must be scaled." % id)
	if is_shape_value and id not in CardPropertySet.SHAPES:
		errors.append("Unknown shape id: %s." % id)
